...
class Scoreboard:
	...
	def prep_ships(self):
		...
		for ship_number in range(self.stats.ships_left + 1):
			...
			ship_width = int(ship.image.get_width() * 0.5)
			ship_height = int(ship.image.get_height() * 0.5)
			ship.image = pygame.transform.scale(ship.image, (ship_width, ship_height))
			ship.rect.x = 10 + ship_number * (ship_width + 5)
			ship.rect.y = 10
			self.ships.add(ship)

	def show_score(self):
		"""Draw scores, ships, and level o the screen."""
